# Limux Post-Install Checklist v2

Version: v2
Status: current; derived from frozen v1 for stable-as-default launcher promotion
...
Launcher resolution for promotion: if a full v2 run passes, promotion installs
the verified source to the stable lane. The installer updates
`~/.local/bin/limux-stable` / `limux-stable-cli` and promotes the plain
`~/.local/bin/limux` / `limux-cli` aliases to the same install. Legacy remains
available through `limux-legacy` / `limux-legacy-cli`; later legacy installs
must not replace the stable default aliases.
...
Versioning rule: v1 remains frozen. This v2 checklist is the current baseline;
future content changes after its first recorded run must create v3. Run files
must name the checklist version and git SHA they executed.
...
not a manual symlink edit. Confirm `limux --version` and `limux doctor --json`
match the promoted stable host, and confirm `limux-legacy --version` still
identifies the rollback lane. If any item fails, do not promote. Record verdict
and evidence, reopen or add TaskMaster tasks, and attach the run file.
...
1. Ensure a daily-driver Limux runtime is open before launching preview. Use
   `~/.local/bin/limux` after stable promotion. On the first verification cycle
   before any stable promotion exists, use the explicit
   `~/.local/bin/limux-legacy` rollback launcher.
