...
| # | Item | What / acceptance sketch | Evidence |
|---|---|---|---|
| W1.1 | **Live-bridge parity core** (architectural unlock) | Implement the designed-but-unbuilt bridge parity layer: GTK host owns a live-synced `ControlState`; `control_bridge` routes its 18 GTK-side-effect methods as today, falls through **read-only** methods to a clone of `limux_core::Dispatcher` state, and adds explicit GTK mutation routes only for PRD-E's Wave-1 live set: `pane.focus`, `pane.resize`/`resize-pane`, `surface.split/focus/close`, `workspace.reorder`, `workspace.next/previous/last`, existing `tab.action` actions, and `notification.clear`. Routes outside that set are explicit deferred `-32601` in Wave 1 (`pane.swap/break/join`, `surface.move/reorder/drag_to_split`, `workspace.move_to_window`, `window.create/close/focus`). Acceptance: Xvfb smoke drives a representative read and live mutation from each accepted family; registry coverage table generated from code. | Parity plan Phase 2 (open since inception); limux inventory §B |
...
| PRD-D `pane-attention` | W0.5 attention border + pane-scoped color flags (#20) — includes the new `pane.action` live-bridge route that doesn't exist yet | M |
