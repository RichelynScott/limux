...
2. **PR #138** (`dame/renderer-tick-overlap-20260812`, `76c0f4f` + `45351ec` + `227fe58` + `b05b05a` + `21be5bf`): fixed the 100 ms hidden-tick overlap with the 8 ms visible frame timer (~10 ticks/s), and added aggregate timer-vs-wakeup counters through health output. The timer counter combines 8 ms + 100 ms timer calls; the remaining ~85/s delta is unattributed pending live measurement. The preserved merged-tree wrapper test passed 497 host tests; after focused #138 was restored, the wrapper content from that ref was rerun against focused #138 and also passed 497 tests. Neither result is live validation.
...
- **Wrapper provenance:** the preserved merged #137 + #138 integration tree passed `scripts/cargo-env.sh test -p limux-host-linux` with 497 tests and 0 failures. After restoring focused #138, the same wrapper content was rerun against focused #138 as `git show dame/renderer-anti-refill-integration:scripts/cargo-env.sh | bash -s test -p limux-host-linux`, again with 497 tests and 0 failures. The focused #138 branch does not contain the #137 wrapper; no live install/restart occurred.
