...
attempt_backend() {
    ...
    # The probe CLIs below are children of this runner, not of the preview host.
    # An inherited LIMUX_SOCKET_MODE/CMUX_SOCKET_MODE of limuxOnly/cmuxOnly would
    # make limux_control::auth::is_descendant reject every probe, so healthy
    # backends would be misreported as unhealthy. Clear the legacy alias and force
    # localUser: same-uid authorization is preserved and the socket stays 0600
    # (SocketControlMode::requires_owner_only_socket).
    setsid env \
